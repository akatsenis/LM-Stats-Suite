...
def report_table(df, caption="", decimals=None):
    ...
    styled = df.style.hide(axis="index").set_caption(caption).set_table_styles([
        ...
        {"selector": "thead th", "props": [("border-top", "1px solid #111827"), ("border-bottom", "1px solid #111827"), ("padding", "8px 12px"), ("text-align", "center"), ("background-color", "#FFFFFF")]},
        {"selector": "tbody td", "props": [("padding", "8px 12px"), ("text-align", "center")]},
        {"selector": "tbody tr:last-child td", "props": [("border-bottom", "1px solid #111827")]},
...
def dis_plot_bootstrap_f2_distribution(boot_vals, observed_f2, ci_low=None, ci_high=None, ci_label="90% CI", title="Distribution of f2 Similarity Factor", x_min=50, x_max=100):
    ...
    ax.text(mean_boot*0.996, y_top * 0.6, f"Mean = {mean_boot:.2f}", ha="right", color=cfg["primary_color"], **text_kw)
    if ci_low is not None:
        ax.text(ci_low*0.996, y_top * 0.6, f"Lower CI = {ci_low:.2f}", ha="right", color=cfg["tertiary_color"], **text_kw)
    if ci_high is not None:
        ax.text(ci_high*1.004, y_top * 0.6, f"Upper CI = {ci_high:.2f}", ha="left", color=cfg["tertiary_color"], **text_kw)
    return fig
