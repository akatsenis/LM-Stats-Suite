...
def dis_plot_bootstrap_f2_distribution(boot_vals, observed_f2, ci_low=None, ci_high=None, ci_label="90% CI", title="Distribution of f2 Similarity Factor", x_min=50, x_max=100):
    ...
    ax.text(mean_boot, y_top * 0.75, f"Mean = {mean_boot:.2f}", ha="right", color=cfg["primary_color"], **text_kw)
    if ci_low is not None:
        ax.text(ci_low, y_top * 0.75, f"Lower CI = {ci_low:.2f}", ha="right", color=cfg["tertiary_color"], **text_kw)
    if ci_high is not None:
        ax.text(ci_high, y_top * 0.75, f"Upper CI = {ci_high:.2f}", ha="left", color=cfg["tertiary_color"], **text_kw)
    return fig
