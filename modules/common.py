...
def dis_plot_bootstrap_f2_distribution(boot_vals, observed_f2, ci_low=None, ci_high=None, ci_label="90% CI", title="Distribution of f2 Similarity Factor", x_min=50, x_max=100):
    ...
    if ci_low is not None:
        ax.text(0.99 * ci_low , y_top * 0.50, f"Lower CI = {ci_low:.2f}", ha="right", color=cfg["tertiary_color"], **text_kw)
    if ci_high is not None:
        ax.text(1.01 * ci_high, y_top * 0.50, f"Upper CI = {ci_high:.2f}", ha="left", color=cfg["tertiary_color"], **text_kw)
    return fig
