        ...
        if data_input:
            try:
                ...
                if len(factors) < 2:
                    ...
                    anova = anova[["Source", "df", "Sum of Squares", "Mean Square", "F-Statistic", "P-Value", "SS (%)"]]
                    ...
                    export_results(
                        ...
                        statistical_analysis=(
                            "A design-of-experiments style regression model was fitted to the selected numeric response using the chosen numeric factors. "
                            "Depending on the selected option, the model included linear terms only, linear plus interactions, or a quadratic response-surface structure. "
                            "ANOVA, model coefficients, contour plots, surface plots, and residual diagnostics were generated from the fitted model."
                        ),
                        offer_text=(
                            "This analysis offers a way to quantify factor effects, inspect interactions, model curvature, and visualize the response surface over two selected factors while fixing any remaining factors at chosen values."
                        ),
                        python_tools=(
                            "Python tools used here include pandas and numpy for selecting factor and response columns, statsmodels for model fitting and ANOVA, matplotlib for contour, 3D surface, and residual diagnostic plots, openpyxl for Excel export, and reportlab for the PDF-style report."
                        ),
