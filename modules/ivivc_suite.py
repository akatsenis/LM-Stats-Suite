...
def render():
    ...
    if tool == "💊 Dissolution Comparison (f₂)":
        ...
        top1 = st.columns(6)
        with top1[0]:
            time_unit_label = st.selectbox("Time units", ["Minutes", "Hours", "Days"], index=0, key="deconv_time_units")
        with top1[1]:
        ...
        with top2[5]:
            model_choice = st.selectbox("Weibull model(s)", MODEL_CHOICE_OPTIONS, index=0, key="deconv_model_choice")
        ...
        st.caption("The selected input time unit is converted internally so all fitting and stored parameters are in hours. The recovered release function is now driven through the ODE with the analytical Weibull input-rate directly, rather than by finite-difference differentiation of the cumulative curve. Fmax remains the sole amplitude parameter, and the editable table below lets you change starts, bounds, and fixed parameters before fitting.")
        ...
        if pk_text:
            try:
                ...
                t_hours_preview = pk_df["Time_input"].to_numpy(dtype=float) * TIME_UNIT_TO_HOURS[time_unit_label]
                y_preview = pk_df.iloc[:, 1:].to_numpy(dtype=float).ravel()
                y_preview = y_preview[np.isfinite(y_preview)]
                parameter_tables = {}
                with st.expander("Parameter bounds, starting values, and fixed parameters", expanded=False):
                    ...
                    default_tables = build_deconv_parameter_tables(t_hours_preview, y_preview)
                    for model_name in MODEL_SPECS:
                        st.markdown(f"**{model_name}**")
                        editor = st.data_editor(
                            _deconv_parameter_long_to_wide(model_name, default_tables[model_name]),
                            key=f"deconv_editor_{_slugify(model_name)}",
                            hide_index=True,
                            num_rows="fixed",
                            width='stretch',
                        )
                        parameter_tables[model_name] = _deconv_parameter_wide_to_long(model_name, pd.DataFrame(editor))
