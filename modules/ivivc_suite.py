...
def fit_pk_deconvolution_suite(pk_df, time_unit_label, disposition, model_choice=None, progress_callback=None):
    factor = TIME_UNIT_TO_HOURS[time_unit_label]
    t_in = pk_df["Time_input"].to_numpy(dtype=float)
    t_h = t_in * factor
    cp_cols = [c for c in pk_df.columns if c != "Time_input"]
    ...
    if tool == "💊 Dissolution Comparison (f₂)":
        ...
        if pk_text:
            try:
                ...
                pack = fit_pk_deconvolution_suite(pk_df, time_unit_label, disposition, model_choice=model_choice, progress_callback=_cb)
